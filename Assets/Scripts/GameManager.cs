...
public class GameManager : MonoBehaviour
{
    ...
    List<GridItem> CheckHoriontalMatches(GridItem item)
    {
    ...
    MatchInfo GetMatchInfo(GridItem item)
    {
        ...
        List<GridItem> horizontalMatch = CheckHoriontalMatches(item);
        List<GridItem> verticalMatch = CheckVerticalMatches(item);

        if (horizontalMatch.Count > verticalMatch.Count && horizontalMatch.Count >= minimumMatch)
        {
        ...
        else if(verticalMatch.Count >= minimumMatch)
        {
    ...
    void OnMouseOverItem(GridItem item)
    {
        ...
        else
        {
            ...
            _selectedItem.GetComponent<Animator>().SetTrigger("Unselect");
            _selectedItem = null;
        }
    }
    ...
    GridItem InstantiateFruit(int xPos, int yPos)
    {
        ...
        GridItem newFruit = ((GameObject)Instantiate(randomFruit, new Vector3(xPos * cellWidth, yPos), Quaternion.identity)).GetComponent<GridItem>();
